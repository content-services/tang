...
func (t *tangyImpl) RpmRepositoryVersionModuleStreamsList(ctx context.Context, hrefs []string, filterOpts ModuleStreamListFilters, SortBy string) ([]ModuleStreams, int, error) {
	if len(hrefs) == 0 {
	...
	orderBy := ""
	if SortBy == "" {
		orderBy = "rp.name, rp.stream"
	} else {
		switch true {
		case strings.Contains(SortBy, "name"):
			orderBy = "rp.name"
		case strings.Contains(SortBy, "stream"):
			orderBy = "rp.stream"
		default:
			orderBy = ""
		}

		if strings.Contains(SortBy, "desc") {
			orderBy += " DESC"
		} else {
			orderBy += " ASC"
		}
	...
	query := `SELECT rp.name, rp.stream, max(rp.version) as version, rp.profiles, rp.context, rp.arch, rp.description FROM rpm_modulemd rp 
	...
	countQueryOpen := `SELECT COUNT(*) FROM (Select distinct name from (` + query
	innerUnion := contentIdsInVersions(repoVerMap, &args)
	...
	filter := rpmNameFilter + " AND rp.name ILIKE CONCAT( '%', @nameFilter::text, '%') GROUP BY rp.name, rp.stream, rp.context,rp.profiles, rp.arch, rp.description"

	var countTotal int

	err = conn.QueryRow(ctx, countQueryOpen+innerUnion+filter+")as listinner) as list", args).Scan(&countTotal)
