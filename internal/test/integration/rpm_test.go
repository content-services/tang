...
func (r *RpmSuite) TestRpmRepositoryVersionModuleStreamsList() {
	...
	// expect empty
	singleList, total, err := r.tangy.RpmRepositoryVersionModuleStreamsList(context.Background(), []string{*firstVersionHref}, tangy.ModuleStreamListFilters{}, "")
	require.NoError(r.T(), err)
	...
	// Test search
	singleList, total, err = r.tangy.RpmRepositoryVersionModuleStreamsList(context.Background(), []string{*firstVersionHref}, tangy.ModuleStreamListFilters{Search: "Duck"}, "")
	require.NoError(r.T(), err)
	assert.NotEmpty(r.T(), singleList)
	assert.Equal(r.T(), 1, total)

	// Test package name list filter
	singleList, total, err = r.tangy.RpmRepositoryVersionModuleStreamsList(context.Background(), []string{*firstVersionHref}, tangy.ModuleStreamListFilters{RpmNames: []string{"walrus", "kangaroo"}}, "")
	require.NoError(r.T(), err)
	assert.NotEmpty(r.T(), singleList)
	assert.Equal(r.T(), 2, total)

	// Confirm no error on not found rpm name
	singleList, total, err = r.tangy.RpmRepositoryVersionModuleStreamsList(context.Background(), []string{*firstVersionHref}, tangy.ModuleStreamListFilters{RpmNames: []string{"banana"}}, "")
	require.NoError(r.T(), err)
	assert.Empty(r.T(), singleList)
